...
- **Inference-time compute allocation** — *when to think, how long, on which model* (budgeted MDPs, routers, cascades)
- **Safe & verifiable autonomy** — CMDPs, Lyapunov/ISS, shields/CBFs, conformal guarantees
- **World models & MARL** — learned simulators, cooperative multi-agent RL (QMIX, MAPPO)
- **AI-native 6G / AI-RAN** — DRL/MARL schedulers, LLM intent layers, digital twins (3GPP Rel-20/21, IMT-2030)
- **Edge & federated learning** — quantization (AWQ/GPTQ/QLoRA), federated LoRA/PEFT, on-device inference

## 🛠️ Tech stack
`PyTorch` · `HF Transformers / PEFT / TRL` · `vLLM` · `llama.cpp` · `AWQ / GPTQ / bitsandbytes` · `LangGraph / CrewAI / MCP` · `Gymnasium / RLlib / CleanRL` · `Sionna / ns-3 / srsRAN` · `FastAPI + Docker` · `AWS / GCP`
